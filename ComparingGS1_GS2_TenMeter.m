...
%Comparing GS1 and GS2 split of 6-meter walking speed.
%December, 2022.
clc;
close all;
clear;
...
%Output results data
Outputpath = '/ResultsComparingTwoSessions/';

%Video Capture Session 1 data path
Videopath = '/ResultsVideo/';
VideoName='S1S10F2Speed';

%Video Capture Session 2 data path
Videopath = '/ResultsVideo/';
VideoName2='S2S10F2Speed';
...
MoCapData = fullfile([Videopath,VideoName2,'.xlsx']);
rawMoCapData = xlsread(MoCapData);

% Defile point 1
%MoCapTime=rawMoCapData(:,1);
MoCapX=rawMoCapData(:,1);
MoCapV=rawMoCapData(:,2);
...
% %A = {'MoCap_Vmax','GS_Vmax','MoCap_Vmin','GS_Vmin','DMoCap_Vmax','DGS_Vmax','DMoCap_Vmin','DGS_Vmin'; VmaxpksT, VmaxpksG, -VminpksT, -VminpksG, 6-MoCapX(VmaxlocsT), 6-GaitDataX(VmaxlocsG),6-MoCapX(VminlocsT),6-GaitDataX(VminlocsG)};
 A = {'Session1(0-1)','Session2(0-1)','Session1(1-2)','Session2(1-2)','Session1(2-3)','Session2(2-3)','Session1(3-4)','Session2(3-4)','Session1(4-5)','Session2(4-5)','Session1(5-6)','Session2(5-6)'; Gait_Mean6, Mocap_Mean6,  Gait_Mean5, Mocap_Mean5,  Gait_Mean4, Mocap_Mean4, Gait_Mean3, Mocap_Mean3, Gait_Mean2,  Mocap_Mean2, Gait_Mean1, Mocap_Mean1};
 sheet = 1;
 xlRange = 'A1';
